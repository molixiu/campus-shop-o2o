$(function() {
	//定义访问后台，获取头条列表的URL
	var urlHeadLine = '/o2o/frontend/headLine/list';
	//定义访问后台，获取一级商店类别列表的URL
	var urlShopCategory = '/o2o/frontend/shopCategory/list';
	
	//访问后台，获取头条列表以及一级类别列表
	$.getJSON(urlHeadLine, function(result) {
		if (result.success) {
			//获取后台传递过来的头条列表
			var headLineList = result.data;
			var swiperHtml = '';
			//遍历头条列表，并拼接出轮播图组
	...
	//访问后台，获一级商店类别列表
	$.getJSON(urlShopCategory, function(result) {
		//获取后台传递过来的大类列表
