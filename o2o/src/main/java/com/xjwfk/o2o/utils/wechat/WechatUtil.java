package com.xjwfk.o2o.utils.wechat;
...
import com.xjwfk.o2o.dto.UserAccessToken;
...
public class WechatUtil {
	...
	/**   
	 * @Title: getWechatUserByCode   
	 * @Description: TODO(通过code获取到access_token 进而获取到用户信息用)   
	 * @param: @param code
	 * @param: @return      
	 * @return: WechatUser      
	 * @throws   
	 */  
	public static WechatUser getWechatUserByCode(String code) {
		WechatUser user = null;
		try {
			if (code != null) {
				// 通过code获取UserAccessToken
				UserAccessToken token = getUserAccessToken(code);
				// 通过UserAccessToken获取accessToken
				String accessToken = token.getAccessToken();
				// 通过UserAccessToken获取openId
				String openId = token.getOpenId();
				// 通过access_token和openId获取用户昵称等信息
				user = getWechatUser(accessToken, openId);
			}
		} catch (Exception e) {
	...
	public static UserAccessToken getUserAccessToken(String code) throws IOException {
		// 测试号信息里的appId
		String appId = "wxcf319137576da744";
		log.debug("appId:" + appId);
		// 测试号信息里的appsecret
		String appsecret = "41ac1f2d1bf3ac1cb178c216bbbbd80b";
	...
	public static PersonInfo getPersonInfoFromRequest(WechatUser user) {
		...
		personInfo.setEmail(user.getCountry()+user.getProvince()+user.getCity());
