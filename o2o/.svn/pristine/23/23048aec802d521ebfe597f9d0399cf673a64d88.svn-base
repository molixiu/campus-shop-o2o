$(function() {
	...
	var pageSize = 3;
	// 列出商品列表的URL
	var lisProductUrl = '/o2o/frontend/product/list';
	// 默认的页码
	var pageNum = 1;
	// 从地址栏里获取ShopId
	...
	var productName = '';
	// 根据shopId获取商品类别信息列表的URL
	var listProductCategoryUrl = '/o2o/frontend/productCategory/list?shopId=' + shopId;
	// 根据shopId获取本店铺信息的URL
	var shopDetailUrl = '/o2o/frontend/shop/detail?shopId=' + shopId;
	...
	function getSearchDivData() {
		$.getJSON(shopDetailUrl,function(result) {	//获取本店铺信息
							if (result.success) {
		...
		$.getJSON(listProductCategoryUrl,function(result) {	//获取本店商品类别信息列表
			if (result.success) {
				// 获取后台返回的该店铺的商品类别列表
	...
	function addItems(pageSize, pageIndex) {
		...
		var url = lisProductUrl + '?' + 'pageIndex=' + pageIndex + '&pageSize='
